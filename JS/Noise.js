document.addEventListener("DOMContentLoaded", () => {
  ...
  if (search) { search.addEventListener("input", updateBreakdown); }
});
...
function buildToolCatalogue() {
  ...
  const grouped = {};
  tools.filter(t => t.category === "preset").forEach(tool => {
...
function buildToolCatalogue(searchTerm = "") {
  ...
  tools.filter(t => t.category === "preset").filter(tool => {
      if (!filter) return true;
      return [tool.manufacturer,tool.type,tool.name]
        .join(" ")
        .toLowerCase()
        .includes(filter);
    })
...
function updateBreakdown() {
  ...
  const grouped = buildToolCatalogue(search);
